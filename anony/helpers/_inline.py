...
class Inline:
    ...
    def queue_markup(
        self, chat_id: int, _text: str, playing: bool, full_url: str | None = None, full_text: str = "View Full List"
    ) -> types.InlineKeyboardMarkup:
        _action = "pause" if playing else "resume"
        rows = [[self.ikb(text=_text, callback_data=f"controls {_action} {chat_id} q")]]
        if full_url:
            rows.append([self.ikb(text=full_text, url=full_url)])
        return self.ikm(rows)
