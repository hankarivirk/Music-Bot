...
async def _queue_func(_, m: types.Message):
    ...
    full_url = None
    if _queue:
        if len(_queue) > QUEUE_INLINE_LIMIT:
            full_list = "\n".join(
                f"{i}. {media.title} - {media.duration}"
                for i, media in enumerate(_queue, start=2)
            )
            full_url = await utils.paste(full_list)

        if full_url:
            _text += m.lang["queue_full_notice"]
        else:
            _text += "<blockquote expandable>"
            for i, media in enumerate(_queue, start=1):
                if i == QUEUE_INLINE_LIMIT:
                    break
    ...
    _buttons = buttons.queue_markup(
            ...
            _playing,
            full_url=full_url,
            full_text=m.lang["view_full_list"],
        )
    if thumb:
